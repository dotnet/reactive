...
namespace Microsoft.Reactive.Async.Testing
{
    ...
    public class TestAsyncScheduler : PriorityQueueVirtualTimeAsyncScheduler<long,long>
    {
        ...
        public async Task<ITestableAsyncObserver<T>> Start<T>(Func<IAsyncObservable<T>> create, long created, long subscribed, long disposed)
        {
            ...
            await ScheduleAbsolute(created, _ => { source = create(); return Task.CompletedTask; });
            await ScheduleAbsolute(subscribed, async _ => { subscription = await source.SubscribeAsync(observer); });
            await ScheduleAbsolute(disposed, async _ => { await subscription.DisposeAsync(); });
