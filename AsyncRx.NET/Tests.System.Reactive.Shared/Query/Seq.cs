...
// sequence, not an observable. Its leaves are the target's own objects (NativeSeq<T>: a
...
// one node type per overload. Nothing here runs. A target (IRxTarget, in the Harness folder) is a
...
// materializes as the target's Window, Select and Merge with nothing in between — the inner
...
// target at once could not do that: it would have to wrap every inner window with a Select and
