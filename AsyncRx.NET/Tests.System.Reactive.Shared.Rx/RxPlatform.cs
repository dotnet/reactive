...
/// <summary>
/// The Rx.NET platform: the environment and raw surface (one line each over
/// <c>TestScheduler</c>) plus the materializer (one line per node). The only casts
/// are of leaves' <c>Native</c>; the pipeline a scenario describes is built by the platform's
/// own operators with nothing in between.
/// </summary>
public sealed class RxPlatform : IPlatform
{
    public static RxPlatform Instance { get; } = new();
    ...
    // ---- Raw surface ----
    //
    // The shared delegates are async-shaped; on this platform everything they can await
    ...
    private static void Complete(ValueTask task)
    {
        if (!task.IsCompleted)
        {
            throw new InvalidOperationException(
                "On the Rx.NET platform every operation a shared scenario can await completes synchronously, but this delegate returned an incomplete task.");
        }
